...
module.exports = (req, res) => {
    ...
    const repno = req.params.repno;

    const queryParams = [studentid, year, companyid, branchname, repno];
    ...
    if (!studentid) {
        ...
        db.query("DELETE FROM report WHERE StudentID = ? AND Year = ? AND CompanyID = ? AND BranchName = ? AND `Number` = ?;", queryParams, (err, results, fields) => {
            if (err) {
                console.log(err);
                res.sendStatus(500);
